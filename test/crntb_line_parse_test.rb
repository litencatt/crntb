...
class LineParseTest < Minitest::Test
  ...
  def test_minute_parse
    ...
    result = Crntb::Line.new('5-10 * * * * foo.sh').to_s
  ...
  def test_day_of_week_parse
    result = Crntb::Line.new('* * * * 3   foo.sh').to_s
    ...
    result = Crntb::Line.new('* * * * 1-3 foo.sh').to_s
  ...
  def test_mix_parse
    result = Crntb::Line.new('0 8,10,12,14,16,18 * * * php emptyTrash.php > log.txt').to_s
    assert_equal result, %Q{every day at 08:00, 10:00, 12:00, 14:00, 16:00, 18:00\n  run command "php emptyTrash.php > log.txt"}

    result = Crntb::Line.new('1-6,9,32 * * * * echo "sample me good" > /etc/tmp/trash.txt').to_s
